...
export interface CohortStudentRow {
  ...
  readonly currentStageCode: SapSuiteE2eStageCode | null;
  readonly institutionalStatus: SapSuiteInstitutionalStatus;
  readonly progressionLabel: string;
  readonly lastUpdateLabel: string;
  readonly difficulty: string;
  ...
  readonly professorNoteCount: number;
}

export function createDefaultStages(
  current: SapSuiteE2eStageCode = "S1",
  status: SapSuiteInstitutionalStatus = "not_started",
): SapSuiteDeclaredStage[] {
  return SAP_SUITE_E2E_STAGE_CODES.map((stageCode) => ({
    stageCode,
    status: stageCode === current ? status : "not_started",
  }));
}
...
export const MOCK_STUDENT_REPORT: StudentSelfReport = {
  ...
  sapAccess: "commence",
  stages: createDefaultStages("S3", "progress_declared"),
  evidence: [],
...
export const MOCK_COHORT: readonly CohortStudentRow[] = [
  {
    ...
    declaredUnit: 3,
    currentStageCode: "S3",
    institutionalStatus: "in_accompaniment",
    evidenceCount: 0,
    professorNoteCount: 0,
  ...
  },
  {
    ...
    access: "commence",
    declaredUnit: 9,
    currentStageCode: "S10",
    institutionalStatus: "accompaniment_completed",
    evidenceCount: 1,
    professorNoteCount: 0,
